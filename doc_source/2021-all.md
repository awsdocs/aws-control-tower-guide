...
+ This release adds a "dead letter" queue for the SNS forwarder Lambda function that already exists in all managed accounts, to forward AWS Config violations to SNS in the Audit account\. Starting with this release, if the Lambda fails to process a Config event, the event is moved to a dead letter queue in the Audit account\. 
