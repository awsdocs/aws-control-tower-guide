...
Identifying and resolving drift is a regular operations task for AWS Control Tower management account administrators\.
...
AWS Control Tower detects drift automatically\. Drift is surfaced in the Amazon Simple Notification Service \(Amazon SNS\) notifications that are aggregated in the audit account\. Notifications in each member account send alerts to a local Amazon SNS topic, and to a Lambda function\.
...
This kind of drift can occur when an AWS Control Tower member account is added to an AWS Control Tower organization\. The following is an example of the Amazon SNS notification when this type of drift is detected\.
...
You can resolve this type of drift by updating the account in Account Factory\. For more information, see [Updating and Moving Account Factory Accounts with AWS Service Catalog](account-factory.md#updating-account-factory-accounts)\.
