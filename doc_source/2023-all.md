...
+ [AWS Control Tower handles accounts based on ID](#email-to-id)
+ [Additional Security Hub detective controls available in the AWS Control Tower controls library](#more-sh-controls)
+ [AWS Control Tower publishes control metadata tables](#publish-metadata)
+ [Terraform support for Account Factory Customization](#afc-terraform)
+ [AWS IAM Identity Center self\-management available for landing zone](#iam-self-manage)
...
## AWS Control Tower handles accounts based on ID<a name="email-to-id"></a>
...
## Additional Security Hub detective controls available in the AWS Control Tower controls library<a name="more-sh-controls"></a>
...
The new AWS Security Hub detective controls are available in all AWS Regions where AWS Control Tower is available\. For more details about these controls, see [Controls that apply to Service\-Managed Standard: AWS Control Tower](https://docs.aws.amazon.com/securityhub/latest/userguide/service-managed-standard-aws-control-tower.html#aws-control-tower-standard-controls)\.

## AWS Control Tower publishes control metadata tables<a name="publish-metadata"></a>
...
AWS Control Tower now provides full tables of control metadata as part of the published documentation\. When working with the control APIs, you can look up each control's **API controlIdentifier**, which is a unique ARN associated with each AWS Region\. The tables include the frameworks and control objectives that each control covers\. Previously, this information was available in the console only\.
 ...
 The tables also include the metadata for Security Hub controls that are part of the [https://docs.aws.amazon.com/controltower/latest/userguide/security-hub-controls.html](https://docs.aws.amazon.com/controltower/latest/userguide/security-hub-controls.html)\. For full details, see [Tables of control metadata](control-metadata-tables.md)\.

For an abbreviated list of control identifiers, and some usage examples, see [Resource identifiers for APIs and controls](control-identifiers.md)\.
