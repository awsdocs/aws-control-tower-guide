...
| --- | --- | --- | 
|  [AWSControlTowerServiceRolePolicy](#AWSControlTowerServiceRolePolicy) – Update to an existing policy  |  AWS Control Tower added new permissions that allow AWS Control Tower to assume the `AWSControlTowerBlueprintAccess` role in the blueprint \(hub\) account, which is a dedicated account in an organization, containing pre\-defined blueprints stored in one or more AWS Service Catalog Products\. AWS Control Tower assumes the `AWSControlTowerBlueprintAccess` role to perform three tasks: create a Service Catalog Portfolio, add the requested blueprint Product, and share the Portfolio to a requested member account at account provisioning time\. This change is needed so that customers can provision customized accounts through AWS Control Tower Account Factory\.  | October 28, 2022 | 
